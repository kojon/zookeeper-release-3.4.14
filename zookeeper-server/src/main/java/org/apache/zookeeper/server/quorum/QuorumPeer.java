...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    synchronized public void startLeaderElection() {
        ...
        for (QuorumServer p : getView().values()) {
            if (p.id == myid) {
                myQuorumAddr = p.addr; //地址： 1 ->myQuorumAddr=192.168.56.185
                break;
            }
        }
        ...
        try {
            ...
            while (running) {
                ...
                case LEADING:
                    LOG.info("LEADING");
                    try {
                        setLeader(makeLeader(logFactory)); //和leader建立socket连接
                        leader.lead(); //构建lead状态下的事务数据  开启消费者和生产者队列
                        setLeader(null);
                    } catch (Exception e) {
                        LOG.warn("Unexpected exception",e);
                    } finally {
